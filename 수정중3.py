...
def split_facet(face_point):
    ...
    for i in face_point:
        ...
        if i==face_point[0]:
            ...
            k+=len(i[2])-1
            face_point2.append([i[0],i[1],list(range(many_node+1,many_node+k))])
            ...
            for j in range(len(i[2])-1):
                ...
                if not j==len(i[2])-2:
                    new_node.append(many_node+k+j)
            face_point2.append([i[0],i[1],list(range(many_node+k-1,many_node+k+len(i[2])-1))])
            k+=len(i[2])-1
                    
    last_k=k-1
    face_point2[0][2]=[many_node+last_k]+face_point2[0][2]
    ...
    face_point2_overlab={}
    for i in prb_i:
        face_point2_overlab[i]=[]
            
    g=1
    for i in range(len(i_list)):
        
        if i_list[i] not in prb_i: # 한번만 지나갔을 때 
            if not face_point2[i][1][0]==face_point2[i][1][1]: # 둘이 다른 변일 때 
                point = [x for x in [1,2,3] if x not in face_point2[i][1]]
                point2 = face_point2[i][1][1]
                for j in range(len(face_point2[i][2])-1):
                    if j==0:
                        facet[many_facet+g]=[1005,face_point2[i][2][j],face_point2[i][2][j+1],
                                             skin_facet[face_point2[i][0]][point[0]-1]]
                        facet[many_facet+g+1]=[1005,face_point2[i][2][j],face_point2[i][2][j+1],
                                             skin_facet[face_point2[i][0]][point2-1]]
                        g+=2
        
                        v1=skin[face_point2[i][2][j+1]]-skin[face_point2[i][2][j]]
                        v1=v1/norm(v1)
                        v2=skin[skin_facet[face_point2[i][0]][point2-1]]-skin[face_point2[i][2][j]]
                        v2=v2/norm(v2)
                        v0=np.cross(v1,v2)
                        ...
                        if np.dot(v0,v3)>0:
                            ...
                            facet[many_facet+g+1]=[1005,face_point2[i][2][j],skin_facet[face_point2[i][0]][point2-1],
                                             skin_facet[face_point2[i][0]][face_point2[i][1][0]-1]]
                            
                            point2=face_point2[i][1][0]
                            facet[many_facet+g+2]=[1005,face_point2[i][2][j],face_point2[i][2][j+1],
                                             skin_facet[face_point2[i][0]][point2-1]]
                            g+=3
                        
                if point2==face_point2[i][1][1]:
                ...
                l=[0,-1]
                for j in l:
                    length.append(norm(skin[skin_facet[face_point2[i][0]][point[0]-1]]-skin[face_point2[i][2][j]]))
                ...
                for j in range(len(face_point2[i][2])-1):
                    if j==0:
                        ...
                        if np.dot(v0,v3)>0:
                            ...
                            elif point2==point2_list[0]: #점 세개만에 통과했을때 나오는 예외 케이스 이다. 
                                facet[many_facet+g]=[1005,face_point2[i][2][j],skin_facet[face_point2[i][0]][point2-1],
                                                     skin_facet[face_point2[i][0]][point2_list[count_p+1]-1]]
                                count_p +=1
                                point2=point2_list[count_p]
                        ...
                        facet[many_facet+g]=[6,face_point2[i][2][j],face_point2[i][2][j+1],
                                             face_point2[i][2][-1]]
                        v1=skin[face_point2[i][2][j+1]]-skin[face_point2[i][2][j]]
                        v1=v1/norm(v1)
                        v2=skin[skin_facet[face_point2[i][0]][point2-1]]-skin[face_point2[i][2][j]]
        ...
        else  : # 두개 지나갈때 (convex한 도형의 경우, 맥시멈 3개까지 이다. 그러나 여기선 일단 2개까지만 구현하도록 한다)
            if len(face_point2_overlab[i_list[i]])==0:
                face_point2_overlab[i_list[i]]=face_point2[i][1:]
            
            else: 
                face_point2_overlab[i_list[i]]=face_point2_overlab[i_list[i]]+face_point2[i][1:]
                '''if not face_point2_overlab[i_list[i]][0][1]==face_point2_overlab[i_list[i]][2][0]:
    ...
    for i in face_point2_overlab:
        if face_point2_overlab[i][0][1]==face_point2_overlab[i][2][0]:
            if not face_point2_overlab[i][0][0]==face_point2_overlab[i][2][1]: # 1번 케이스 ex) [2,3][3,1]
                point = [x for x in [1,2,3] if x not in face_point2_overlab[i][0]]
                for j in range(len(face_point2_overlab[i][1])-1):
                    if j<len(face_point2_overlab[i][1])-2:
                        facet[many_facet+g]=[1005,face_point2_overlab[i][1][j],face_point2_overlab[i][1][j+1],
                                                     skin_facet[i][point[0]-1]]
                        facet[many_facet+g+1]=[1005,face_point2_overlab[i][1][j],face_point2_overlab[i][1][j+1],
                                                     face_point2_overlab[i][1][-1]]
                        g+=2
                    else:
                        facet[many_facet+g]=[1005,face_point2_overlab[i][1][j],face_point2_overlab[i][1][j+1],
                                                     skin_facet[i][point[0]-1]]
                ...
                point = [x for x in [1,2,3] if x not in face_point2_overlab[i][2]]
                for j in range(len(face_point2_overlab[i][3])-1):
                    facet[many_facet+g]=[1005,face_point2_overlab[i][3][j],face_point2_overlab[i][3][j+1],
                                                     skin_facet[i][point[0]-1]]
                    facet[many_facet+g+1]=[1005,face_point2_overlab[i][3][j],face_point2_overlab[i][3][j+1],
                                                     face_point2_overlab[i][1][-1]]
                    g+=2
        
                facet[many_facet+g]=[1005,face_point2_overlab[i][1][-1],face_point2_overlab[i][3][-1],
                                                     skin_facet[i][face_point2_overlab[i][0][1]-1]]
                facet[many_facet+g+1]=[1005,face_point2_overlab[i][1][-1],face_point2_overlab[i][1][0],
                                                     skin_facet[i][face_point2_overlab[i][0][1]-1]]
                g+=2
                
            else : # 3번 케이스 ex) [2,3], [3,2] 케이스이다. 상당히 어려움 
                point = [x for x in [1,2,3] if x not in face_point2_overlab[i][0]]
                
                length=[]
                length.append(norm(skin[skin_facet[i][point[0]-1]]-skin[face_point2_overlab[i][1][-1]]))
                length.append(norm(skin[skin_facet[i][point[0]-1]]-skin[face_point2_overlab[i][3][0]]))
                    
                small=length.index(min(length))
                if small==0:
                    pass
                else :
                    face_point2_overlab[i]=[face_point2_overlab[i][2],face_point2_overlab[i][3],
                                            face_point2_overlab[i][0],face_point2_overlab[i][1]]
                ...
                facet[many_facet+g]=[1005,face_point2_overlab[i][1][-1],face_point2_overlab[i][3][-1],
                                                     face_point2_overlab[i][1][0]]
                g+=1
                
                point2 = face_point2_overlab[i][2][1]
                for j in range(len(face_point2_overlab[i][3])-1):
                    if j==0:
                        facet[many_facet+g]=[1005,face_point2_overlab[i][3][j],face_point2_overlab[i][3][j+1],
                                             skin_facet[i][point2-1]]
                        
                        g+=1
        
                        v1=skin[face_point2_overlab[i][3][j+1]]-skin[face_point2_overlab[i][3][j]]
                        v1=v1/norm(v1)
                        v2=skin[skin_facet[i][point2-1]]-skin[face_point2_overlab[i][3][j]]
                        v2=v2/norm(v2)
                        v0=np.cross(v1,v2)
                        ...
                        if np.dot(v0,v3)>0:
                            ...
                            facet[many_facet+g]=[1005,face_point2_overlab[i][3][j], skin_facet[i][point2-1],
                                                 skin_facet[i][face_point2_overlab[i][2][0]-1]]
                            
                            point2=face_point2_overlab[i][2][0]
                            facet[many_facet+g+1]=[1005,face_point2_overlab[i][3][j],face_point2_overlab[i][3][j+1],
                                             skin_facet[i][point2-1]]
                            g+=2
                        
                if point2==face_point2_overlab[i][2][1]:
                    facet[many_facet+g]=[1005,face_point2_overlab[i][3][-1],skin_facet[i][face_point2_overlab[i][2][0]-1],
                                             skin_facet[i][point2-1]]
                    g+=1
        
        else : #2번 케이스 ex) [3,1][2,3] (나중에 1번 케이스의 순서변형 문제로 바꾸고 싶음. 코드가 간단하게 줄어들기 때문에.. )
            point = [x for x in [1,2,3] if x not in face_point2_overlab[i][0]]
            for j in range(len(face_point2_overlab[i][1])-1):
                if j>0:
                    facet[many_facet+g]=[1005,face_point2_overlab[i][1][j],face_point2_overlab[i][1][j+1],
            ...
            facet[many_facet+g]=[1005,face_point2_overlab[i][1][0],face_point2_overlab[i][3][0],
                                                 skin_facet[i][face_point2_overlab[i][0][0]-1]]
            facet[many_facet+g+1]=[1005,face_point2_overlab[i][1][-1],face_point2_overlab[i][1][0],
                                                 skin_facet[i][face_point2_overlab[i][0][0]-1]]
            g+=2
    ...
    return new_node, i_list


'''def when_two_time(face_point2_overlab,i,g):'''
...
#-------------필요없는 부분 제거하는 함수 ----------------------------------------------------------------+
def delete_facet(del_facet_list):
    del_facet2=set(del_facet_list)
...
def plat_num(click_num):   
    ...
    if plat_type==2:
        edge_xy=25 # 사각형 패치의 xy 방향 극판 중심 사이의 거리 
        edge_z=36 # 사각형 패치의 z축 방향 극판 중심 사이의 거리
    elif plat_type==1:
        edge_xy=36 # 사각형 패치의 xy 방향 극판 중심 사이의 거리 
        edge_z=25 # 사각형 패치의 z축 방향 극판 중심 사이의 거리
