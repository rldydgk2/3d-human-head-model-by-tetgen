...
edge_xy=21 # 사각형 패치의 xy 방향 극판 중심 사이의 거리 
...
def head_surface():
    ...
    skin_node_num=list(skin.keys())[0] #스킨 노드의 첫번째 노드 넘버 
    ...
    skin_facet_num=list(skin_facet.keys())[0] #스킨 페이스의 첫번째 노드 넘버 
    return node, facet, region, skin, skin_node_num, skin_facet, skin_facet_num 


node, facet, region, skin, skin_node_num, skin_facet, skin_facet_num =head_surface()
dic={}
...
def make_mass_point_of_facet():
    mass_p_dic={}
    for i in range(len(skin_facet)):
        mass_p=(skin[skin_facet[skin_facet_num+i][0]]
        +skin[skin_facet[skin_facet_num+i][1]]+skin[skin_facet[skin_facet_num+i][2]])/3
        mass_p_dic[skin_facet_num+i]=mass_p
    return mass_p_dic
...
def n_vector(i):  
    ...
    i2=int(l.index(i)/3)
    facet_number=i2+1
    p1=skin[facet[facet_number][1]]
    p2=skin[facet[facet_number][2]]
    p3=skin[facet[facet_number][3]]
    v1=p2-p1
    v2=p3-p1
    n_v=np.cross(v1, v2)
    mag = norm(n_v)
...
def output_region(j,x,y,z,v): # 극판의 리전을 만드는 것 
    ...
    region[3*j+1]=[point1[0],point1[1],point1[2],1] # 리전 1 은 하이드로 갤
    region[3*j+2]=[point2[0],point2[1],point2[2],2] # 리전 2 은 세라믹 
    region[3*j+3]=[point3[0],point3[1],point3[2],3] # 리전 3 은 구리
...
def opposit_side(i,n_v):
    ...
    dot=[]
    for j in range(len(point_vec)):
        if not norm(point_vec[j])==0:
            dot.append([skin_node_num+j,abs(np.dot(n_v,point_vec[j]))])
                
    dot2=[]
    for j in dot:
        dot2.append(j[1])
    k=dot2.index(max(dot2))
    i2=dot[k][0]
...
def make_hydrogel(interest_facet,neighbor,new_node_in_circle,v,many_dic):
    ...
    print('face=%s'%len(face))
    print('point=%s'%len(point))
    face_point={}
    for u in range(len(point)):
        if u == 0:
            ...
            face_point[out][0].append(case)
            face_point[out][1].append(p0)
            
            while not out1==face[u+1]:
                case=find_neighbor(neighbor,out1,out)
                face_point[out1]=[[case],[p0]]
                out2, case, p0=determine_direction(neighbor,out1,p,p0,v)
                face_point[out1][0].append(case)
                face_point[out1][1].append(p0)
                out=out1
                out1=out2
            ...
            while not out1==face[0]:
                ...
                ax.scatter(p0[0], p0[1], p0[2], color='black')
            case=find_neighbor(neighbor,out1,out)
            face_point[out1][0]=[case]+face_point[out1][0]
            face_point[out1][1]=[p0]+face_point[out1][1]
    
    new_node=split_facet(face_point)
...
def split_facet(face_point):
    ...
    for i in face_point:
        if len(face_point[i][1])==2:
            
            point = [x for x in [1,2,3] if x not in face_point[i][0]]
            #node[many_node+k]=[2]+list(face_point[i][1][0]) #LEFT CROSS POINT
            node[many_node+k]=[2]+list(face_point[i][1][1]) #RIGHT CROSS POINT
            k+=1

        elif len(face_point[i][1])==3:
            
            point = [x for x in [1,2,3] if x not in face_point[i][0]]
            #node[many_node+k]=[2]+list(face_point[i][1][0])#LEFT CROSS POINT
            node[many_node+k]=[2]+list(face_point[i][1][1])
            new_node.append(many_node+k)
            node[many_node+k+1]=[2]+list(face_point[i][1][2]) #RIGHT CROSS POINT
            k+=2
    ...
    for i in face_point:
        if len(face_point[i][1])==2:
            del facet[i]
            point = [x for x in [1,2,3] if x not in face_point[i][0]]
            facet[many_facet+g]=[1005,many_node+k-1,many_node+k,skin_facet[i][point[0]-1]]
            facet[many_facet+g+1]=[1005,many_node+k-1,skin_facet[i][face_point[i][0][1]-1],
                                   skin_facet[i][face_point[i][0][0]-1],many_node+k]
            
            k+=1
            g+=2
        
        elif len(face_point[i][1])==3:
            if i==list(face_point.keys())[0]:
                del facet[i]   
                point = [x for x in [1,2,3] if x not in face_point[i][0]]
                facet[many_facet+g]=[1005,many_node+last_k,many_node+k,skin_facet[i][point[0]-1]]
                facet[many_facet+g+1]=[1005,many_node+k,many_node+k+1,skin_facet[i][point[0]-1]]

                facet[many_facet+g+2]=[1005,many_node+last_k,skin_facet[i][face_point[i][0][1]-1],
                                   many_node+k]
                facet[many_facet+g+3]=[1005,many_node+k,skin_facet[i][face_point[i][0][0]-1],
                                   many_node+k+1]
                facet[many_facet+g+4]=[1005,skin_facet[i][face_point[i][0][1]-1],
                                   skin_facet[i][face_point[i][0][0]-1],many_node+k]
                ...
                facet[many_facet+g+2]=[1005,many_node+k-1,skin_facet[i][face_point[i][0][1]-1],
                                   many_node+k]
                facet[many_facet+g+3]=[1005,many_node+k,skin_facet[i][face_point[i][0][0]-1],
                                   many_node+k+1]
                facet[many_facet+g+4]=[1005,skin_facet[i][face_point[i][0][1]-1],
                ...
                k+=2
                g+=5
    '''new_node.append(many_node)
    new_node.append(many_node+k-1)'''
...
def make_interesting_facet(i,detec_l):
    ...
    for j in range(len(mass_p_dic)):
        if norm(point-mass_p_dic[skin_facet_num+j])**2<= detec_l**2:
            interest_facet[skin_facet_num+j]=skin_facet[skin_facet_num+j]
