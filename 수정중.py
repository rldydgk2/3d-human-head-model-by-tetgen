...
##-----------패러미터--------------------------------------------------+
switch=1 #1개 붙일지 모두 붙일지 선택하는 변수 
...
def triangle(skin,skin_facet):
    element=[]
    for i in skin_facet:
        element.append([skin[skin_facet[i][0]],skin[skin_facet[i][1]],skin[skin_facet[i][2]]])
    ax.add_collection3d(Poly3DCollection(element, 
    facecolors='cyan', linewidths=0.1, edgecolors='y', alpha=0.1))
    plt.show()
#triangle(skin,skin_facet)
...
def on_dblclick(event,switch,i,s,d_1,d_2,a):
    ...
    global click_num 
    edge_xy, edge_z=plat_num(click_num)
...
def make_hydrogel(interest_facet,neighbor,new_node_in_circle,v,many_dic):
    ...
    for u in range(len(face)):
        if u<len(face)-1:
            if not face[u]==face[u+1]:
                change.append(u)
        else :
            if not face[u]==face[0]:
                change.append(u)
    ...
    for u in change:
        ...
        if u == change[0]:
        ...
        elif change[0]< u < change[len(change)-1]:
            case=find_neighbor(neighbor,out1,out)
            l2=[out1,[case],[p0]]
            p0=point[u]
            p=point[u+1]
            out=face[u]
            index=change.index(u)
            l2[2]=l2[2] + point[change[index-1]+1:u+1]
            out1, case, p0=determine_direction(neighbor,out,p,p0,v)
            l2[1].append(case)
            l2[2].append(p0)
            face_point.append(l2)
            ...
            if not u==len(point)-1:
                face_point[0][2]=[p0]+point[u+1:]+face_point[0][2]
                revice_k=len(point[u+1:])
            else :
    ...
    side_face(new_node_in_circle, new_node,revice_k)
...
def split_facet(face_point):
    ...
    for i in face_point:
        ...
        for j in range(len(i[2])-1):
            node[many_node+k+j]=[1005]+list(i[2][j+1]) #RIGHT CROSS POINT
            if not j==len(i[2])-2:
                new_node.append(many_node+k+j)
        k+=len(i[2])-1
    ...
    for i in face_point:
        ...
        if n==1:
            ...
            if len(i[2])-2*n==0: # 짝수개 일떄 
                if not i[1][0]==i[1][1]: 
                    if i[0]==face_point[0][0]: # nomal-0
                        del_facet.append(i[0])
                        point = [x for x in [1,2,3] if x not in i[1]]
                        facet[many_facet+g]=[2,many_node+last_k,many_node+k,skin_facet[i[0]][point[0]-1]]
                        for j in range(len(i[2])-2):
                            facet[many_facet+g+1+j]=[1,many_node+k+j, many_node+k+j+1, skin_facet[i[0]][point[0]-1]]
                            
                        g+=len(i[2])-1
                        facet[many_facet+g]=[2,many_node+last_k,many_node+k,skin_facet[i[0]][i[1][1]-1]]
                        facet[many_facet+g+1]=[3,many_node+k+2*n-2,many_node+k+2*n-3,skin_facet[i[0]][i[1][0]-1]]
                        for j in range(n-2):
                            facet[many_facet+g+2+2*j]=[4,many_node+k+j,many_node+k+j+1,skin_facet[i[0]][i[1][1]-1]]
                            facet[many_facet+g+3+2*j]=[5,many_node+k+2*n-3-j,many_node+k+2*n-4-j,skin_facet[i[0]][i[1][0]-1]]
                                
                        facet[many_facet+g+2*(n-1)]=[6,skin_facet[i[0]][i[1][1]-1],many_node+k+n-2,
                                                     skin_facet[i[0]][i[1][0]-1]]
                        facet[many_facet+g+2*(n-1)+1]=[7,many_node+k+n-2,
                                                     many_node+k+n-1,skin_facet[i[0]][i[1][0]-1]]
                        ...
                        point = [x for x in [1,2,3] if x not in i[1]]
                        for j in range(len(i[2])-1):
                            facet[many_facet+g+j]=[1,many_node+k-1+j, many_node+k+j, skin_facet[i[0]][point[0]-1]]
                                    
                        g+=len(i[2])-1
                                    
                        for j in range(n-1):
                            facet[many_facet+g+2*j]=[2,many_node+k-1+j,many_node+k+j,skin_facet[i[0]][i[1][1]-1]]
                            facet[many_facet+g+1+2*j]=[3,many_node+k+2*n-2-j,many_node+k+2*n-3-j,skin_facet[i[0]][i[1][0]-1]]
                                        
                        facet[many_facet+g+2*(n-1)]=[4,skin_facet[i[0]][i[1][1]-1],many_node+k+n-2,
                                                     skin_facet[i[0]][i[1][0]-1]]
                        facet[many_facet+g+2*(n-1)+1]=[5,many_node+k+n-2,
                                                     many_node+k+n-1,skin_facet[i[0]][i[1][0]-1]]
                    ...
                    prb_i.append([k,g,i[0]])
                    if i[0]==face_point[0][0]: # problem-0
                        del_facet.append(i[0])
                        point = [x for x in [1,2,3] if x not in i[1]]
                        length=[]
                        l=[0,1]
                        for j in l:
                            length.append(norm(skin[skin_facet[i[0]][point[j]-1]]-skin[many_node+last_k]))
                                                
                        small=length.index(min(length))
                        big=length.index(max(length))
                                                
                        facet[many_facet+g]=[3,many_node+last_k, many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+1]=[3,many_node+k+2*n-2,many_node+k+2*n-3, skin_facet[i[0]][point[big]-1]]
                        facet[many_facet+g+2]=[3,skin_facet[i[0]][i[1][0]-1], many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+3]=[3,skin_facet[i[0]][i[1][0]-1],many_node+k+2*n-3, skin_facet[i[0]][point[big]-1]]
                                                
                                                
                        for j in range(len(i[2])-3):
                            facet[many_facet+g+4+j]=[3,many_node+k+j, many_node+k+j+1, skin_facet[i[0]][i[1][0]-1]]
                                                    
                        g+=len(i[2])+1
                                                    
                        for j in range(n-2):
                            facet[many_facet+g+2*j]=[3,many_node+last_k,many_node+k+j,many_node+k+1+j]
                            facet[many_facet+g+1+2*j]=[3,many_node+k+2*n-2,many_node+k+2*n-3-j,many_node+k+2*n-4-j]
                            
                        facet[many_facet+g+2*(n-1)]=[3,many_node+last_k,many_node+k+n-2,
                                                     many_node+k+2*n-2]
                        facet[many_facet+g+2*(n-1)+1]=[3,many_node+k+n-2,
                                                     many_node+k+n-1,many_node+k+2*n-2]
                        ...
                        for j in l:
                            ...
                            length.append(norm(skin[skin_facet[i[0]][point[j]-1]]-skin[many_node+k-1]))
                                                            
                        small=length.index(min(length))
                        big=length.index(max(length))
                                                            
                        facet[many_facet+g]=[4,many_node+k-1, many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+1]=[4,many_node+k+2*n-2,many_node+k+2*n-3, skin_facet[i[0]][point[big]-1]]
                        facet[many_facet+g+2]=[4,skin_facet[i[0]][i[1][0]-1], many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+3]=[4,skin_facet[i[0]][i[1][0]-1],many_node+k+2*n-3, skin_facet[i[0]][point[big]-1]]
                                                            
                        for j in range(len(i[2])-3):
                            facet[many_facet+g+4+j]=[4,many_node+k+j, many_node+k+j+1, skin_facet[i[0]][i[1][0]-1]]
                                                                
                        g+=len(i[2])+1
                                                                
                        for j in range(n-2):
                            facet[many_facet+g+2*j]=[4,many_node+k-1,many_node+k+j,many_node+k+1+j]
                            facet[many_facet+g+1+2*j]=[4,many_node+k+2*n-2,many_node+k+2*n-3-j,many_node+k+2*n-4-j]
                                                                    
                        facet[many_facet+g+2*(n-1)]=[4,many_node+k-1,many_node+k+n-2,
                                                     many_node+k+2*n-2]
                        facet[many_facet+g+2*(n-1)+1]=[4,many_node+k+n-2,
                                                     many_node+k+n-1,many_node+k+2*n-2]
            ...
            elif len(i[2])-2*n==1: # 홀수개 일떄 
                if not i[1][0]==i[1][1]:
                                                                            
                    if i[0]==face_point[0][0]: # nomal_0
                        del_facet.append(i[0])
                        point = [x for x in [1,2,3] if x not in i[1]]
                        facet[many_facet+g]=[5,many_node+last_k,many_node+k,skin_facet[i[0]][point[0]-1]]
                        for j in range(len(i[2])-2):
                            facet[many_facet+g+1+j]=[5,many_node+k+j, many_node+k+j+1, skin_facet[i[0]][point[0]-1]]
                                                                                
                        g+=len(i[2])-1
                        facet[many_facet+g]=[5,many_node+last_k,many_node+k,skin_facet[i[0]][i[1][1]-1]]
                        facet[many_facet+g+1]=[5,many_node+k+2*n-1,many_node+k+2*n-2,skin_facet[i[0]][i[1][0]-1]]
                        for j in range(n-1):
                            facet[many_facet+g+2+2*j]=[5,many_node+k+j,many_node+k+j+1,skin_facet[i[0]][i[1][1]-1]]
                            facet[many_facet+g+3+2*j]=[5,many_node+k+2*n-2-j,many_node+k+2*n-3-j,skin_facet[i[0]][i[1][0]-1]]
                                                                                    
                        facet[many_facet+g+2*n]=[5,skin_facet[i[0]][i[1][1]-1],many_node+k+n-1,
                                                skin_facet[i[0]][i[1][0]-1]]
                        ...
                        point = [x for x in [1,2,3] if x not in i[1]]
                        for j in range(len(i[2])-1):
                            facet[many_facet+g+j]=[6,many_node+k-1+j, many_node+k+j, skin_facet[i[0]][point[0]-1]]
                    
                        g+=len(i[2])-1
                
                        for j in range(n):
                            facet[many_facet+g+2*j]=[6,many_node+k-1+j,many_node+k+j,skin_facet[i[0]][i[1][1]-1]]
                            facet[many_facet+g+1+2*j]=[6,many_node+k+2*n-1-j,many_node+k+2*n-2-j,skin_facet[i[0]][i[1][0]-1]]
            
                        facet[many_facet+g+2*n]=[6,skin_facet[i[0]][i[1][1]-1],many_node+k+n-1,
                                                     skin_facet[i[0]][i[1][0]-1]]
                    ...
                    if i[0]==face_point[0][0]:  # problem-0
                        ...
                        facet[many_facet+g]=[7,many_node+last_k, many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+1]=[7,many_node+k+2*n-1,many_node+k+2*n-2, skin_facet[i[0]][point[big]-1]]
                        facet[many_facet+g+2]=[7,skin_facet[i[0]][i[1][0]-1], many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+3]=[7,skin_facet[i[0]][i[1][0]-1],many_node+k+2*n-2, skin_facet[i[0]][point[big]-1]]
                       
                        for j in range(len(i[2])-3):
                            facet[many_facet+g+4+j]=[7,many_node+k+j, many_node+k+j+1, skin_facet[i[0]][i[1][0]-1]]
                    
                        g+=len(i[2])+1
                    
                        for j in range(n-1):
                            facet[many_facet+g+2*j]=[7,many_node+last_k,many_node+k+j,many_node+k+1+j]
                            facet[many_facet+g+1+2*j]=[7,many_node+k+2*n-1,many_node+k+2*n-2-j,many_node+k+2*n-3-j]
            
                        facet[many_facet+g+2*(n-1)]=[7,many_node+last_k,
                                                     many_node+k+n-1,many_node+k+2*n-1]
                        ...
                        facet[many_facet+g]=[8,many_node+k-1, many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+1]=[8,many_node+k+2*n-1,many_node+k+2*n-2, skin_facet[i[0]][point[big]-1]]
                        facet[many_facet+g+2]=[8,skin_facet[i[0]][i[1][0]-1], many_node+k, skin_facet[i[0]][point[small]-1]]
                        facet[many_facet+g+3]=[8,skin_facet[i[0]][i[1][0]-1],many_node+k+2*n-2, skin_facet[i[0]][point[big]-1]]
                    
                  
                        for j in range(len(i[2])-3):
                            facet[many_facet+g+4+j]=[8,many_node+k+j, many_node+k+j+1, skin_facet[i[0]][i[1][0]-1]]
                    
                        g+=len(i[2])+1
                    
                        for j in range(n-1):
                            facet[many_facet+g+2*j]=[8,many_node+k-1,many_node+k+j,many_node+k+1+j]
                            facet[many_facet+g+1+2*j]=[8,many_node+k+2*n-1,many_node+k+2*n-2-j,many_node+k+2*n-3-j]
            
                        facet[many_facet+g+2*(n-1)]=[8,many_node+k-1,
                                                     many_node+k+n-1,many_node+k+2*n-1]
    ...
    for i in prb_i:
        if i[0]==1:
            ...
            p1=face_point[p-1]
            p2=face_point[p+1]
            ip=p1[0] #=p2[0] #p1[1][1]=p2[1][0]
            point = [x for x in [1,2,3] if x not in p1[1]]
            for j in range(len(p1[2])-1):
                facet[many_facet+g+2*j]=[1005,many_node+last_k+1-len(p1[2])+j, many_node+last_k+1-len(p1[2])+1+j,
                                       skin_facet[ip][point[0]-1]]
                facet[many_facet+g+2*j+1]=[1005,many_node+last_k+1-len(p1[2])+j, many_node+last_k+1-len(p1[2])+1+j,
                                       skin_facet[ip][p1[1][1]-1]]
            
            g+=2*(len(p1[2])-1)
            point = [x for x in [1,2,3] if x not in p2[1]]
            for j in range(len(p2[2])-1):
                facet[many_facet+g+2*j]=[1005,many_node+ki+len(p0[2])-2+j, many_node+ki+len(p0[2])-1+j,
                                       skin_facet[ip][point[0]-1]]
                facet[many_facet+g+2*j+1]=[1005,many_node+ki+len(p0[2])-2+j, many_node+ki+len(p0[2])-1+j,
                                       skin_facet[ip][p2[1][0]-1]]
            
            g+=2*(len(p2[2])-1)
                
            facet[many_facet+g]=[1005,skin_facet[ip][p2[1][0]-1],many_node+last_k,many_node+ki+len(p0[2])-2]
            
            g+=1
            
            gi=i[1]
            
            for j in range(2*len(p1[2])-1):
                del_facet.append(many_facet+last_g-j)
    
            g0=2*len(p0[2])-1
    
            for j in range(2*len(p2[2])-1):
                del_facet.append(many_facet+gi+g0+j)
            ...
            point = [x for x in [1,2,3] if x not in p1[1]]
            for j in range(len(p1[2])-1):
                facet[many_facet+g+2*j]=[1005,many_node+ki-len(p1[2])+j, many_node+ki-len(p1[2])+1+j,
                                       skin_facet[ip][point[0]-1]]
                facet[many_facet+g+2*j+1]=[1005,many_node+ki-len(p1[2])+j, many_node+ki-len(p1[2])+1+j,
                                       skin_facet[ip][p1[1][1]-1]]
            ...
            facet[many_facet+g]=[1005,skin_facet[ip][p2[1][0]-1],many_node+ki-1,many_node+ki+len(p0[2])-2]
            ...
            for j in range(2*len(p1[2])-1):
                del_facet.append(many_facet+gi-1-j)
    ...
    for j in range(g-1):
        ...
        mass_p=(skin[skin_facet[many_facet+1+j][0]]+skin[skin_facet[many_facet+1+j][1]]+skin[skin_facet[many_facet+1+j][2]])/3
...
#------------옆면 만드는 함수 ----------------------------------------------------------------+
def side_face(new_node_in_circle, new_node,revice_k):
    print(len(new_node_in_circle))
    print(len(new_node))
    many_node=max(node.keys())
    many_facet=max(facet.keys())
    g=0
    revice_new_node_in_circle=list(new_node_in_circle[revice_k:])+list(new_node_in_circle[:revice_k])
    
    for i in range(len(revice_new_node_in_circle)):
        if i<len(revice_new_node_in_circle)-1:
            k=0
            while not new_node[i]+k==new_node[i+1]:            
                facet[many_facet+g+1]=[-1, revice_new_node_in_circle[i], new_node[i]+k, new_node[i]+k+1]
                k+=1
                g+=1
            facet[many_facet+g+1]=[-1, revice_new_node_in_circle[i], new_node[i+1], revice_new_node_in_circle[i+1]]
            g+=1
            ...
            facet[many_facet+g+1]=[-1, revice_new_node_in_circle[i], new_node[i]+k, new_node[0]]
            facet[many_facet+g+2]=[-1, revice_new_node_in_circle[i], new_node[0], revice_new_node_in_circle[0]]
...
#------------플레이트 길이 조정하는 함수-------------------------------------------------------------+
def plat_num(click_num):   
    plat_type=1+int(click_num/2)
    
    if plat_type==2:
        edge_xy=22 # 사각형 패치의 xy 방향 극판 중심 사이의 거리 
        edge_z=36 # 사각형 패치의 z축 방향 극판 중심 사이의 거리
    elif plat_type==1:
        edge_xy=36 # 사각형 패치의 xy 방향 극판 중심 사이의 거리 
        edge_z=22 # 사각형 패치의 z축 방향 극판 중심 사이의 거리
        
    return edge_xy, edge_z
